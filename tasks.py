...
def coverage(c):
    ...
    """
    c.run("coverage run --source django-ftp-handler runtests.py tests")
...
def docs(c):
    ...
    """
    c.run("rm -f docs/django-ftp-handler.rst")
    c.run("rm -f docs/modules.rst")
    c.run("sphinx-apidoc -o docs/ ftp_handler")
...
def lint(c):
    ...
    """
    c.run("flake8 django-ftp-handler tests")
...
def release(c, bumpsize=''):
    ...
    c.run('git tag -a {version} -m "New version: {version}"'.format(version=ftp_handler.__version__))
    c.run("git push --tags")
